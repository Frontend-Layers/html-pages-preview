...
const htmlPreview = (src, dest,
  ...
  <div class="container-fluid">
    <div class="pt-2 pb-2"${cfg.backToIndex ? `` : ` style="display: none"`}>
      <a href="${cfg.backToIndexUrl ? `${cfg.backToIndexUrl}` : `./index.html`}">Back To Index</a>
    </div>
  ...
  for (let i = 0; i < src.length; i++) {
    ...
    <div class="hpp">
      <div class="hpp-header"${cfg.pane ? `` : ` style="display: none;"`}>
        <div class="container-fluid">
          <div class="pt-4 pb-2">
            <h2 id="${baseFileName}">${cfg.filename ? `${baseFileName}` : ''}${cfg.title ? `${title ? `${cfg.filename ? ' - ' : ''}${title}` : ''}` : ``}</h2>

            <ul>
              ${cfg.toggle ? `<li class="hpp-link-toggle"><a href="#" data-target="toggle">Show/hide</a></li>` : ''}
              ${cfg.newtab ? `<li class="hpp-link-newtab"><a href="${cfg.baseUrl}${baseFileName}" target="_blank">Open in new tab</a></li>` : ''}
            </ul>
      ...
      <div class="hpp-content">
        <iframe class="hpp-iframe" src="${cfg.baseUrl}${baseFileName}"></iframe>
      </div>
    </div>
    `);
    } catch (err) {
      console.error('Page Preview: ' + err.message);
      continue;
    }
